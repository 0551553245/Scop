...
export default function BranchManagerMonthlyTasks() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const [taskRes, subRes] = await Promise.all([
        supabaseBranchManager.from('tasks').select('id, name, name_ar, frequency, category, requires_photo, requires_note, requires_value, value_unit').eq('is_active', true).or(`branch_id.eq.${branchId},and(branch_id.is.null,created_by.eq.${ownerId})`).eq('frequency', 'monthly').order('created_at', { ascending: true }),
        supabaseBranchManager.from('task_submissions').select('id, task_id, status, note, value_entered, submitted_at').eq('branch_id', branchId).eq('submitted_by', profile.id).gte('submitted_at', `${monthStart}T00:00:00.000Z`).lte('submitted_at', `${today}T23:59:59.999Z`).limit(1000),
      ])
  ...
  useEffect(() => {
    ...
    const ch = supabaseBranchManager
      ...
      }, () => { invalidateCache(cacheKey); fetchTasks() })
      .subscribe()
    return () => supabaseBranchManager.removeChannel(ch)
  ...
  if (loading) return (
    <BMLayout activePath="/branch-manager/monthly-tasks" title="Monthly Tasks" titleAr="المهام الشهرية"
      subtitle={new Date().toLocaleDateString(isAr?'ar-SA':'en-US', { month:'long', year:'numeric' })}
      branchName={branchName}>
      <div style={{ padding:'16px 20px' }}>
        {['a','b','c','d'].map(k => <div key={k} className="skeleton" style={{ height:68, marginBottom:8 }} />)}
      </div>
    </BMLayout>
  )
      ...
      <div style={{ padding:'16px 20px' }}>
          ...
          {tasks.length > 0 && (
            <div style={{ marginBottom:16 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                <span style={{ fontSize:13, fontWeight:500, color:'#111827' }}>{isAr ? 'تقدم الشهر' : "This month's progress"}</span>
          ...
          {tasks.length === 0 && (
            <div style={{ background:'#fff', border:'0.5px solid #E5E7EB', borderRadius:12, padding:40, textAlign:'center', marginTop:8 }}>
              ...
              <div style={{ fontSize:14, fontWeight:500, color:'#111827', marginBottom:6 }}>{isAr ? 'لا توجد مهام شهرية' : 'No monthly tasks assigned'}</div>
              <div style={{ fontSize:12, color:'#9CA3AF' }}>{isAr ? 'مالك المطعم لم يُضف مهام شهرية بعد' : "Your restaurant owner hasn't added any monthly tasks yet"}</div>
            </div>
          )}
