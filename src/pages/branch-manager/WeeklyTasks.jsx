...
export default function BranchManagerWeeklyTasks() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const [taskRes, subRes] = await Promise.all([
        supabaseBranchManager.from('tasks').select('id, name, name_ar, frequency, category, requires_photo, requires_note, requires_value, value_unit').eq('is_active', true).or(`branch_id.eq.${branchId},and(branch_id.is.null,created_by.eq.${ownerId})`).eq('frequency', 'weekly').order('created_at', { ascending: true }),
        supabaseBranchManager.from('task_submissions').select('id, task_id, status, note, value_entered, submitted_at').eq('branch_id', branchId).eq('submitted_by', profile.id).gte('submitted_at', `${weekStart}T00:00:00.000Z`).lte('submitted_at', `${today}T23:59:59.999Z`).limit(1000),
      ])
  ...
  if (loading) return (
    <BMLayout activePath="/branch-manager/weekly-tasks" title="Weekly Tasks" titleAr="المهام الأسبوعية"
      subtitle={new Date().toLocaleDateString(isAr?'ar-SA':'en-US', { month:'short', day:'numeric' })}
      ...
      <div style={{ padding:'16px 20px' }}>
          ...
          {tasks.length > 0 && (
            <div style={{ marginBottom:16 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                <span style={{ fontSize:13, fontWeight:500, color:'#111827' }}>{isAr ? 'تقدم الأسبوع' : "This week's progress"}</span>
          ...
          {tasks.length === 0 && (
            <div style={{ background:'#fff', border:'0.5px solid #E5E7EB', borderRadius:12, padding:40, textAlign:'center', marginTop:8 }}>
              ...
              <div style={{ fontSize:14, fontWeight:500, color:'#111827', marginBottom:6 }}>{isAr ? 'لا توجد مهام أسبوعية' : 'No weekly tasks assigned'}</div>
              <div style={{ fontSize:12, color:'#9CA3AF' }}>{isAr ? 'مالك المطعم لم يُضف مهام أسبوعية بعد' : "Your restaurant owner hasn't added any weekly tasks yet"}</div>
            </div>
          )}
